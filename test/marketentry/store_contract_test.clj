...
(defn- exercise [s]
  (store/commit-record! s {:effect :engagement/upsert
                           :value {:id "eng-x" :operator "X S.r.l." :jurisdiction "SMR"
                                   :base-fee 100 :monthly-rate 10 :monitoring-months 1
                                   :claimed-fee 110.0
                                   :non-resident? true :sm-tax-address-established? true
                                   :industrial-licence-referenced? true
  ...
  (store/commit-record! s {:effect :assessment/set
                           :path ["eng-x"]
                           :payload {:jurisdiction "SMR" :checklist ["a"] :spec-basis "x"}})
