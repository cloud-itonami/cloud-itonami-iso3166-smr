  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Titano Servizi S.r.l."}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Titano Servizi S.r.l." (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest non-resident-tax-address-missing-is-held-and-unoverridable
  (testing "non-resident bidder without a San Marino tax address/industrial-licence reference -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:non-resident-tax-address-missing} (-> (store/ledger db) last :basis)))
...
(deftest operating-licence-unverified-is-held-and-unoverridable
  (testing "unverified operating licence (Legge 129/2010) when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:operating-licence-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
