...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (SMR, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up non-resident-tax-address-missing) ==")
    ...
    (println "== filing/submit eng-4 (non-resident-tax-address-missing -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up operating-licence-unverified) ==")
    ...
    (println "== filing/submit eng-5 (operating-licence-unverified -> HARD hold) ==")
