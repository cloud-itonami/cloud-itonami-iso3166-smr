(ns marketentry.marketentryllm
  ...
  the San Marino (SMR) public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "non-resident?=" (:non-resident? e)
                        " sm-tax-address-established?=" (:sm-tax-address-established? e)
                        " industrial-licence-referenced?=" (:industrial-licence-referenced? e)
                        " requires-operating-licence?=" (:requires-operating-licence? e)
                        " operating-licence-verified?=" (:operating-licence-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-operating-licence? e))
                              (:operating-licence-verified? e)))
                   0.9 0.3)}))
